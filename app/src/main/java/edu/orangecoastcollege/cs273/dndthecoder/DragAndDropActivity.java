...
/**
 * OnTouchListener for the "Elevation and Shadow"
 *
 * OnDragListener for the "for SOURCE to know when pic is there"
 */
public class DragAndDropActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //get var for "for loop" var equals all the linearLayouts in in gridLayout
        int llChildCount = dragLayout.getChildCount();

        for(int i = 0; i < llChildCount; i++)
        {
            //get the LinearLayout at each index and
            View childView = dragLayout.getChildAt(i);
            if(childView instanceof LinearLayout)
            {
                //parse to LinearLayout and set the Drag Listener
                LinearLayout childLinearLayout = (LinearLayout)childView;
                childLinearLayout.setOnDragListener(new CoderDragListener());

                int imageViewChildCount = childLinearLayout.getChildCount();
                for(int j = 0; j< imageViewChildCount; j++){
                    View grandCildImageView = childLinearLayout.getChildAt(j);

                    if(grandCildImageView instanceof ImageView)
                    {
                        ImageView childImageView = (ImageView) grandCildImageView;
        ...
        //TODO:  Add the CoderTouchListener to every ImageView within each LinearLayou
    }
